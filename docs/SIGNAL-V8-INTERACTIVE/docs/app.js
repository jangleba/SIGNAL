(() => {
  ...
  document.querySelector("[data-confirm]").addEventListener("click", () => {
    ...
    const phone = document.getElementById("customerPhone").value.trim();
    const now = new Date();
    const eventTime = new Intl.DateTimeFormat("pl-PL", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }).format(now);
    const consentText = `Chcę, aby wyłącznie ${offer.company} skontaktował się ze mną w sprawie „${state.need}”, aby wykonać działanie „${state.action}”, przez ${state.channel.toLowerCase()}, w terminie ${state.slot.toLowerCase()}.`;
    if (window.SignalStore) {
      confirmedRecord = window.SignalStore.saveRecord({
        id: existingLeadMode && params.get("lead") ? params.get("lead") : undefined,
        name,
        phone,
        status: "new",
        statusLabel: "Nowe",
        ...
        entryKey,
        placement,
        origin: existingLeadMode ? "Istniejący lead uzupełniony przez SIGNAL" : "Bezpośrednio przez SIGNAL",
        consent: consentText,
        opened: eventTime,
        selected: eventTime,
        confirmed: eventTime,
    ...
    if (!confirmedRecord) setText("[data-handoff-copy]", "Przeglądarka zablokowała zapis lokalny. Sam proces działa, ale ta karta nie pojawi się w panelu po przejściu dalej.");
    ...
    setText("[data-pass-id]", confirmedRecord ? confirmedRecord.id : `SG-${String(Date.now()).slice(-6)}`);
    history.length = 0;
  ...
  document.querySelector("[data-cancel-action]").addEventListener("click", () => {
    if (confirmedRecord && window.SignalStore) {
      confirmedRecord = window.SignalStore.saveRecord({ ...confirmedRecord, status: "cancelled", statusLabel: "Anulowane", phone: "numer ukryty po anulowaniu", action: "Nie kontaktować", time: "Anulowano" });
    }
    history.length = 0;
    showStage("cancelled", false);
  });

  document.querySelectorAll("[data-close-offer]").forEach(control => control.addEventListener("click", event => {
    event.preventDefault();
    if (window.history.length > 1 && document.referrer) window.history.back();
    else window.location.href = "zamkniete.html";
  }));
