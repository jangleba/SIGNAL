(() => {
  ...
  const tap = () => { if (navigator.vibrate) navigator.vibrate(12); };
  ...
  $$('[data-intent]').forEach(b=>b.addEventListener('click',()=>{tap();show(b.dataset.intent==='interested'?'details':b.dataset.intent)}));
  ...
  $$('[data-need]').forEach(b=>b.addEventListener('click',()=>{tap();state.need=b.dataset.need;select('[data-need]',b);validateDetails()}));
  $$('[data-action]').forEach(b=>b.addEventListener('click',()=>{tap();state.action=b.dataset.action;state.duration=b.dataset.duration;select('[data-action]',b);validateDetails()}));
  ...
  $$('[data-slot]').forEach(b=>b.addEventListener('click',()=>{tap();state.slot=b.dataset.slot;select('[data-slot]',b);renderSummary()}));
